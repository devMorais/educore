-- ============================================================
-- EduCore — Tarefa META (Caderno = espelho do CLAUDE.md)
-- Rodar uma unica vez. Se a tarefa META-01 ja existir, isso so atualiza o Caderno (notes).
-- ============================================================

-- 1) Cria a tarefa META se ainda nao existir
INSERT INTO tasks (board_id, sprint_id, status_id, assigned_to, description, notes, priority, epic, sort_order, created_at, updated_at)
SELECT 7, (SELECT id FROM sprints WHERE board_id = 7 AND name LIKE 'Sprint 1 -%' LIMIT 1), 25, 1, '[META-01] Contexto do Projeto para IA (CLAUDE.md)
...
Se algum dia os dois ficarem diferentes, o arquivo do repositório manda — o Caderno aqui é só um espelho pra quem está sem o repo aberto.', '# EduCore — Contexto para IA
...
', 'Alta', 'Meta: Documentacao Viva', -1, NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM tasks WHERE board_id = 7 AND description LIKE '[META-01]%');

-- 2) Se ja existir, garante que o Caderno (notes) fica sincronizado com o CLAUDE.md atual do repo
UPDATE tasks SET notes = '# EduCore — Contexto para IA
...
| `DEPLOY.private.md` | Credenciais e troubleshooting de deploy (local, não versionado) |
| `sql-sprints/*.sql` | Scripts de atualização das descrições das demandas por sprint no Avante |
', updated_at = NOW() WHERE board_id = 7 AND description LIKE '[META-01]%';

-- 3) Cria a tag "Meta" se ainda nao existir, e vincula
INSERT INTO tags (board_id, name, color, created_at, updated_at)
SELECT 7, 'Meta', '#64748B', NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM tags WHERE board_id = 7 AND name = 'Meta');

INSERT INTO task_tag (task_id, tag_id)
SELECT t.id, tg.id FROM tasks t JOIN tags tg ON tg.board_id = 7 AND tg.name = 'Meta'
WHERE t.board_id = 7 AND t.description LIKE '[META-01]%'
AND NOT EXISTS (SELECT 1 FROM task_tag WHERE task_id = t.id AND tag_id = tg.id);

-- 4) Verificacao
SELECT id, LEFT(description,50) AS titulo, LENGTH(notes) AS tamanho_caderno FROM tasks WHERE board_id = 7 AND description LIKE '[META-01]%';